...
export const Sidebar: React.FC<SidebarProps> = ({ selectedAccount }) => {
  ...
  const { pendingCount } = useTransactions(selectedAccount.id);
  ...
  // Nav items defined here so Approve can show live badge
  const NAV = [
    {
      label: 'Home',
      href: '/home',
    ...
    },
    {
      label: 'Approve',
      href: '/approve',
      badge: pendingCount > 0 ? pendingCount : null, // live badge
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
    ...
    <aside className="w-full h-full bg-[#111] border-r border-gray-800 flex flex-col">
      ...
      <div className="p-5 border-b border-gray-800 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#7FFFD4] text-sm font-mono ml-auto px-1.5 py-0.5
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        {NAV.map((item) => (
          ...
          >
            ...
            {/* Live pending badge */}
            {item.badge !== null && (
              <span className={`text-sm font-bold px-1.5 py-0.5 rounded-full shrink-0
                ${isActive(item.href)
        ...
        {isOwner ? (
          <div className="pt-4">
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2 text-gray-500
                         text-sm uppercase tracking-wider hover:text-gray-300 transition-colors"
            >
              <span>Settings</span>
              <svg
          ...
          <div className="pt-4">
            <div className="flex items-center gap-2 px-3 py-2 text-gray-700 text-sm
                            uppercase tracking-wider cursor-not-allowed select-none">
      ...
      <div className="p-4 border-t border-gray-800 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-500 text-sm">Sepolia testnet</span>
        </div>
        <p className="text-gray-700 text-sm mt-1 font-mono truncate">
