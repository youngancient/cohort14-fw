...
export function PropertyRow({ property, onList, onUnlist, onDelete }: PropertyRowProps) {
  ...
  const status = property.isSold
    ...
    : property.isListed
    ? <Badge variant="listed"><span className="material-symbols-outlined filled text-[12px]">check_circle</span> Listed</Badge>
    : <Badge variant="unlisted"><span className="material-symbols-outlined text-[12px]">inventory_2</span> Vaulted</Badge>;
      ...
      <td className="px-4 md:px-6 py-5">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-on-surface-variant font-label">ID: OP-{property.id.toString().padStart(4, '0')}</p>
          </div>
        </div>
      </td>
      <td className="hidden md:table-cell px-6 py-5">
        <div className="flex flex-col">
          <span className="text-sm font-medium font-body">{property.propType}</span>
          <span className="text-xs text-on-surface-variant font-body">{property.category}</span>
        </div>
      </td>
      <td className="hidden sm:table-cell px-6 py-5">
        <p className="font-headline font-bold text-primary">{formatPrice(property.price)}</p>
      </td>
